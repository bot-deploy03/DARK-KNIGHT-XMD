...
cmd({
  ...
  try {
    ...
    if (!searchData) {
      const searchUrl = `https://suhas-bro-api.vercel.app/movie/zoom/search?text=${encodeURIComponent(q)}`;
      let retries = 3;
      while (retries > 0) {
        try {
          const searchResponse = await axios.get(searchUrl, { timeout: 10000 });
    ...
    const selectionHandler = async (update) => {
      ...
      if (repliedToId === movieListMessageKey.id) {
        ...
        // Fetch download links and details
        const downloadUrl = `https://suhas-bro-api.vercel.app/movie/zoom/movie?url=${encodeURIComponent(selectedFilm.link)}`;
        let downloadData;
        let downloadRetries = 3;
...
cmd({
  ...
  try {
    ...
    if (!searchData) {
      const searchUrl = `https://www.dark-yasiya-api.site/movie/firemovie/search?text=${encodeURIComponent(q)}`;
      let retries = 3;
      while (retries > 0) {
        try {
          const searchResponse = await axios.get(searchUrl, { timeout: 10000 });
    ...
    const selectionHandler = async (update) => {
      ...
      if (repliedToId === movieListMessageKey.id) {
        ...
        // Fetch download links and details
        const downloadUrl = `https://www.dark-yasiya-api.site/movie/firemovie/movie?url=${encodeURIComponent(selectedFilm.link)}`;
        let downloadData;
        let downloadRetries = 3;
